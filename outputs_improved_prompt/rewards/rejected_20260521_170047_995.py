def _get_rewards(self) -> torch.Tensor:
    ...
    orientation = torch.atan2(self._robot.data.root_ang_vel_b[:, 1], self._robot.data.root_ang_vel_b[:, 2])

    distance_to_goal_mapped = 1 - torch.tanh(distance_to_goal / 0.8)
    height_reward = height_reward_scale * (self.height_target - height)
    orientation_reward = orientation_reward_scale * torch.abs(orientation)
