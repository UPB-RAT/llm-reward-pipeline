def _get_rewards(self) -> torch.Tensor:
    ...
    angular_smoothness_reward_scale = -0.005
    vertical_smoothness_reward_scale = -0.002
    ...
    angular_smoothness = torch.sum(torch.abs(self._robot.data.root_ang_vel_b[:, 2]), dim=1)
    vertical_smoothness = torch.sum(torch.abs(self._robot.data.root_lin_vel_b[:, 2]), dim=1)
    ...
    rewards = {
        ...
        "vertical_smoothness": vertical_smoothness * vertical_smoothness_reward_scale * self.step_dt,
    }
