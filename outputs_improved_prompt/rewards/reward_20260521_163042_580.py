def _get_rewards(self) -> torch.Tensor:
    ...
    angular_displacement_reward_scale = -0.1
    velocity_magnitude_reward_scale = -0.01
    ...
    angular_displacement = torch.sum(torch.abs(self._robot.data.root_ang_vel_b), dim=1)
    velocity_magnitude = torch.sum(torch.abs(self._robot.data.root_lin_vel_b), dim=1)
    ...
    rewards = {
        ...
        "angular_displacement": angular_displacement * angular_displacement_reward_scale * self.step_dt,
        "velocity_magnitude": velocity_magnitude * velocity_magnitude_reward_scale * self.step_dt,
    }
