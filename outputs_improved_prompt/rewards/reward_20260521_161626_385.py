def _get_rewards(self) -> torch.Tensor:
    ...
    angular_error_reward_scale = -0.02
    height_stability_reward_scale = -0.01

    lin_vel = torch.sum(torch.square(self._robot.data.root_lin_vel_b), dim=1)
    ang_vel = torch.sum(torch.square(self._robot.data.root_ang_vel_b), dim=1)
    distance_to_goal = torch.linalg.norm(self._desired_pos_w - self._robot.data.root_pos_w, dim=1)
    tilt = torch.sum(torch.square(self._robot.data.projected_gravity_b[:, :2]), dim=1)
    height = self._robot.data.root_pos_w[:, 2] - self._desired_pos_w[:, 2]
    angular_error = torch.sum(torch.abs(self._robot.data.root_ang_vel_b), dim=1)

    height_stability = torch.tanh(height / 0.5)
    angular_error_mapped = torch.tanh(angular_error / 1.0)
    ...
    rewards = {
        ...
        "angular_error": angular_error_mapped * angular_error_reward_scale * self.step_dt,
        "height_stability": height_stability * height_stability_reward_scale * self.step_dt,
    }
