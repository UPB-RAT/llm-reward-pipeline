def _get_rewards(self) -> torch.Tensor:
    ...
    orientation = torch.sum(torch.square(self._robot.data.root_ang_vel_b[:, :2]), dim=1)

    distance_to_goal_mapped = 1 - torch.tanh(distance_to_goal / 0.8)
    height_error = torch.abs(height - self.desired_pos_w[:, 2])
    orientation_error = torch.sum(torch.square(self._robot.data.root_ang_vel_b[:, :2]), dim=1)
    ...
    rewards = {
        ...
        "height": height_error * height_reward_scale * self.step_dt,
        "orientation": orientation_error * orientation_reward_scale * self.step_dt,
    }
