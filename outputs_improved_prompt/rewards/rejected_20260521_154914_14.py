def _get_rewards(self) -> torch.Tensor:
    ...
    smoothness_reward_scale = -0.01

    lin_vel = torch.sum(torch.square(self._robot.data.root_lin_vel_b), dim=1)
    ang_vel = torch.sum(torch.square(self._robot.data.root_ang_vel_b), dim=1)
    distance_to_goal = torch.linalg.norm(self._desired_pos_w - self._robot.data.root_pos_w, dim=1)
    height = torch.abs(self._robot.data.root_pos_w[:, 2] - self._desired_pos_w[:, 2])
    tilt = torch.sum(torch.square(self._robot.data.projected_gravity_b[:, :2]), dim=1)
    smoothness = torch.sum(torch.square(self._robot.data.root_ang_vel_b[:, 0]), dim=1)

    distance_to_goal_mapped = 1 - torch.tanh(distance_to_goal / 0.8)
    height_reward = height * height_reward_scale * self.step_dt
    tilt_reward = tilt * tilt_reward_scale * self.step_dt
    smoothness_reward = smoothness * smoothness_reward_scale * self.step_dt
