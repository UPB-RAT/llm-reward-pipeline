def _get_rewards(self) -> torch.Tensor:
    ...
    height_error = torch.abs(self._robot.data.root_pos_w[:, 2] - self._desired_pos_w[:, 2])
    tilt = torch.sum(torch.square(self._robot.data.projected_gravity_b[:, :2]), dim=1)
    angular_error = torch.sum(torch.abs(self._robot.data.root_ang_vel_b - torch.tensor([0.0, 0.0, 0.0], dtype=torch.float32).unsqueeze(0).repeat(self._robot.data.root_ang_vel_b.shape[0], 1)), dim=1)
    ...
    rewards = {
        ...
        "angular_error": angular_error * angular_error_reward_scale * self.step_dt,
    }
