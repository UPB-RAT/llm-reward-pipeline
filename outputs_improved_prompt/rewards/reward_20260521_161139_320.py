def _get_rewards(self) -> torch.Tensor:
    ...
    angular_jitter_reward_scale = -0.02
    horizontal_speed_reward_scale = 0.03
    ...
    angular_jitter = torch.sum(torch.abs(self._robot.data.root_ang_vel_b), dim=1)
    horizontal_speed = torch.sqrt(torch.sum(torch.square(self._robot.data.root_lin_vel_b[:, :2]), dim=1))

    # Assemble dictionary (ESCAPED BRACES HERE)
    rewards = {
        ...
        "angular_jitter": angular_jitter * angular_jitter_reward_scale * self.step_dt,
        "horizontal_speed": horizontal_speed * horizontal_speed_reward_scale * self.step_dt,
    }
