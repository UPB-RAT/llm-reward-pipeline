def _get_rewards(self) -> torch.Tensor:
    ...
    orientation = torch.sum(torch.abs(self._robot.data.projected_gravity_b[:, :2]), dim=1)
    
    distance_to_goal_mapped = 1 - torch.tanh(distance_to_goal / 0.8)
    height_mapped = torch.tanh(height / 0.5)
    orientation_mapped = torch.tanh(orientation / 0.2)
