def _get_rewards(self) -> torch.Tensor:
    ...
    orientation = torch.norm(self._robot.data.root_ang_vel_b, dim=1)

    distance_to_goal_mapped = 1 - torch.tanh(distance_to_goal / 0.8)
    height_error = torch.abs(height - self._desired_pos_w[:, 2])
    orientation_error = torch.abs(orientation - self.desired_ang_vel_w[:, 2])
