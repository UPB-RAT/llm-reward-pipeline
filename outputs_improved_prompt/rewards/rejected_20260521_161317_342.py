def _get_rewards(self) -> torch.Tensor:
    ...
    height_error_mapped = torch.tanh(torch.abs(height_error) / 1.0)
    tilt_error = torch.sum(torch.square(self._robot.data.root_ang_vel_b[:, 2]), dim=1)
    ...
    rewards = {
        ...
        "tilt_error": tilt_error * tilt_reward_scale * self.step_dt,
    }
