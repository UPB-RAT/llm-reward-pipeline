def _get_rewards(self) -> torch.Tensor:
    ...
    motor_efficiency_reward_scale = -0.01
    ...
    height_reward = height_reward_scale * (self.height_target - height) ** 2
    tilt = torch.sum(torch.square(self._robot.data.projected_gravity_b[:, :2]), dim=1)
    motor_efficiency = torch.sum(torch.square(self._robot.data.projected_gravity_b), dim=1)
    ...
    rewards = {
        ...
        "motor_efficiency": motor_efficiency * motor_efficiency_reward_scale * self.step_dt,
    }
