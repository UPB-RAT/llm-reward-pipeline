def _get_rewards(self) -> torch.Tensor:
    ...
    angular_rate_reward_scale = -0.03
    ...
    angular_rate = torch.sum(torch.square(self._robot.data.root_ang_vel_b), dim=1)

    distance_to_goal_mapped = 1 - torch.tanh(distance_to_goal / 0.8)
    height_mapped = torch.tanh(height - self.desired_pos_w[:, 2]) * height_reward_scale
    ...
    rewards = {
        ...
        "height": height_mapped * self.step_dt,
        "tilt": tilt * tilt_reward_scale * self.step_dt,
        "angular_rate": angular_rate * angular_rate_reward_scale * self.step_dt,
    }
