def _get_rewards(self) -> torch.Tensor:
    ...
    velocity_diff_reward_scale = -0.03
    acceleration_diff_reward_scale = -0.02
    ...
    velocity_diff = torch.sum(torch.abs(self._robot.data.root_lin_vel_b - self._desired_vel_w), dim=1)
    acceleration_diff = torch.sum(torch.abs(self._robot.data.root_lin_acc_b - self._desired_acc_w), dim=1)
    ...
    rewards = {
        ...
        "velocity_diff": velocity_diff * velocity_diff_reward_scale * self.step_dt,
        "acceleration_diff": acceleration_diff * acceleration_diff_reward_scale * self.step_dt,
    }
