def _get_rewards(self) -> torch.Tensor:
    ...
    energy = torch.sum(torch.square(self._robot.data.root_lin_vel_b) + torch.square(self._robot.data.root_ang_vel_b), dim=1)

    distance_to_goal_mapped = 1 - torch.tanh(distance_to_goal / 0.8)
    height_reward = height_reward_scale * height
    tilt_reward = tilt_reward_scale * tilt
    energy_reward = energy_reward_scale * energy
    ...
    rewards = {
        ...
        "height": height_reward * self.step_dt,
        "tilt": tilt_reward * self.step_dt,
        "energy": energy_reward * self.step_dt,
    }
