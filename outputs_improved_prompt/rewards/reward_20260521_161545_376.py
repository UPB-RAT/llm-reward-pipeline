def _get_rewards(self) -> torch.Tensor:
    ...
    rotational_stability_reward_scale = -0.005
    ...
    height_reward = height_reward_scale * (1 - torch.tanh(height / 5.0))
    tilt = torch.sum(torch.square(self._robot.data.projected_gravity_b[:, :2]), dim=1)
    rotational_stability = torch.sum(torch.abs(self._robot.data.root_ang_vel_b), dim=1)
    ...
    rewards = {
        ...
        "rotational_stability": rotational_stability * rotational_stability_reward_scale * self.step_dt,
    }
