def _get_rewards(self) -> torch.Tensor:
    ...
    height_diff = torch.abs(height - self.desired_pos_w[:, 2])
    tilt = torch.sum(torch.square(self._robot.data.projected_gravity_b[:, :2]), dim=1)
    energy = torch.sum(torch.square(self._robot.data.root_ang_vel_b) + torch.square(self._robot.data.root_lin_vel_b), dim=1)
    ...
    rewards = {
        ...
        "height": height_diff * height_reward_scale * self.step_dt,
        "tilt": tilt * tilt_reward_scale * self.step_dt,
        "energy": energy * energy_reward_scale * self.step_dt,
    }
