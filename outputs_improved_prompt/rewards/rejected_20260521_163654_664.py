def _get_rewards(self) -> torch.Tensor:
    ...
    height_loss = torch.abs(height - self.desired_pos_w[:, 2])
    tilt_loss = torch.sum(torch.square(self._robot.data.root_ang_vel_b[:, 2]), dim=1)
    ...
    rewards = {
        ...
        "height": height_loss * height_reward_scale * self.step_dt,
        "tilt": tilt_loss * tilt_reward_scale * self.step_dt,
    }
