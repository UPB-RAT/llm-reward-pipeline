def _get_rewards(self) -> torch.Tensor:
    ...
    angular_deviation_reward_scale = -0.02
    lin_accel_reward_scale = -0.03
    ...
    angular_deviation = torch.sum(torch.abs(self._robot.data.root_ang_vel_b[:, 2]), dim=1)
    lin_accel = torch.sum(torch.square(self._robot.data.root_lin_vel_b[:, 2]), dim=1)
    ...
    rewards = {
        ...
        "angular_deviation": angular_deviation * angular_deviation_reward_scale * self.step_dt,
        "lin_accel": lin_accel * lin_accel_reward_scale * self.step_dt,
    }
