def _get_rewards(self) -> torch.Tensor:
    ...
    rew_pole_angle = rew_scale_pole_pos * torch.abs(torch.atan2(pole_pos, cart_pos))
    rew_cart_force = rew_scale_cart_vel * torch.abs(self.cartpole.data.joint_effort_target[:, self._cart_dof_idx])
    rewards = {
        "alive": rew_alive,
