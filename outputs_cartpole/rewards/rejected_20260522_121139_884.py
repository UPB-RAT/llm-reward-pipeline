def _get_rewards(self) -> torch.Tensor:
    ...
    rew_cart_pos = rew_scale_pole_pos * torch.exp(-torch.abs(cart_pos))
    rew_pole_accel = rew_scale_pole_vel * torch.abs(self.joint_acc[:, self._pole_dof_idx[0]])
    rewards = {
        "alive": rew_alive,
