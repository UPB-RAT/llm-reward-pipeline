def _get_rewards(self) -> torch.Tensor:
    ...
    rew_pole_pos = rew_scale_pole_pos * torch.exp(-torch.abs(pole_pos))
    rew_cart_vel = rew_scale_cart_vel * torch.exp(-torch.abs(cart_vel))
    rew_pole_vel = rew_scale_pole_vel * torch.exp(-torch.abs(pole_vel))
    rew_balance = rew_scale_pole_pos * (1.0 - torch.abs(pole_angle))
    rewards = {
        "alive": rew_alive,
