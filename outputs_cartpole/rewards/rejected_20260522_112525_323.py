def _get_rewards(self) -> torch.Tensor:
    ...
    rew_pole_angle = rew_scale_pole_pos * torch.abs(pole_pos)
    rew_cart_acc = rew_scale_cart_vel * torch.abs(torch.diff(cart_vel, prepend=cart_vel[0], append=cart_vel[-1])) / self.step_dt
    rewards = {
        "alive": rew_alive,
