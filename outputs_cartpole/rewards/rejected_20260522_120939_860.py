def _get_rewards(self) -> torch.Tensor:
    ...
    rew_pole_angle = rew_scale_pole_pos * torch.square(torch.abs(pole_pos) - math.pi / 2)
    rew_cart_accel = rew_scale_cart_vel * torch.abs(torch.sum(self.joint_acc[:, self._cart_dof_idx], dim=1))
    rewards = {
        "alive": rew_alive,
