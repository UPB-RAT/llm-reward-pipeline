def _get_rewards(self) -> torch.Tensor:
    ...
    rew_cart_angle = rew_scale_pole_pos * torch.abs(torch.atan2(pole_pos, cart_pos))
    rew_cart_jerk = rew_scale_cart_vel * torch.abs(torch.diff(cart_vel, dim=0, prepend=torch.zeros_like(cart_vel[:, :1])))
    rewards = {
        "alive": rew_alive,
