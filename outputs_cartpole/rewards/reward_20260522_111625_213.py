def _get_rewards(self) -> torch.Tensor:
    ...
    rew_pole_angle = rew_scale_pole_pos * torch.exp(-torch.abs(pole_pos))
    rew_cart_jerk = rew_scale_cart_vel * torch.abs(self.joint_vel[:, self._cart_dof_idx[0]] - self.joint_vel[:, self._cart_dof_idx[0] - 1])
