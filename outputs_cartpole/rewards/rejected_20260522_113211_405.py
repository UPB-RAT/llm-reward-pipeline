def _get_rewards(self) -> torch.Tensor:
    ...
    rew_cart_pos = -torch.exp(-torch.abs(cart_pos)) * self.step_dt
    rew_pole_acc = -torch.abs(torch.diff(pole_vel, prepend=pole_vel[0].unsqueeze(0), append=pole_vel[-1].unsqueeze(0)) / self.step_dt) * self.step_dt
    rewards = {
        "alive": rew_alive,
