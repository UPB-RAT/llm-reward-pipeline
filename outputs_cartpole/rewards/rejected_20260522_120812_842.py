def _get_rewards(self) -> torch.Tensor:
    ...
    rew_pole_pos = rew_scale_pole_pos * torch.exp(-torch.abs(pole_pos) ** 2)
    rew_cart_vel = rew_scale_cart_vel * torch.abs(cart_vel)
    rew_pole_vel = rew_scale_pole_vel * torch.abs(pole_vel)
    rew_cart_angle = rew_scale_pole_pos * torch.exp(-torch.abs(self.joint_pos[:, self._cart_dof_idx[0]]) ** 2)
