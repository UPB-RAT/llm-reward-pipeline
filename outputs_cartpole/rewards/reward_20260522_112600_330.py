def _get_rewards(self) -> torch.Tensor:
    ...
    rew_pole_angle = rew_scale_pole_pos * torch.abs(pole_pos) * (1.0 - torch.exp(-torch.abs(pole_pos)))
    rew_cart_smoothness = rew_scale_cart_vel * torch.exp(-torch.abs(cart_vel) ** 2)
