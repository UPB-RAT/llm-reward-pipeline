def _get_rewards(self) -> torch.Tensor:
    ...
    rew_pole_angle = rew_scale_pole_pos * torch.abs(pole_pos) * torch.exp(-torch.abs(pole_pos))
    rew_cart_angle = rew_scale_cart_vel * torch.abs(cart_pos) * torch.exp(-torch.abs(cart_pos))
    rewards = {
        "alive": rew_alive,
