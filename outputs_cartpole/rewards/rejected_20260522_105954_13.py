def _get_rewards(self) -> torch.Tensor:
    ...
    rew_pole_angle = rew_scale_pole_pos * torch.clamp(0.5 - torch.abs(pole_pos) / torch.pi, 0, 1)
    rew_cart_angle = rew_scale_cart_vel * torch.clamp(0.5 - torch.abs(cart_pos) / self.cfg.max_cart_pos, 0, 1)
    rewards = {
        "alive": rew_alive,
