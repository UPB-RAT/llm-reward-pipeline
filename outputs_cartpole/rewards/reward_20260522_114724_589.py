def _get_rewards(self) -> torch.Tensor:
    ...
    rew_pole_angle = rew_scale_pole_pos * torch.abs(pole_pos) * torch.exp(-torch.abs(pole_vel))
    rew_cart_balance = rew_scale_cart_vel * torch.exp(-torch.abs(cart_pos)) * torch.abs(cart_vel)
    rewards = {
        "alive": rew_alive,
