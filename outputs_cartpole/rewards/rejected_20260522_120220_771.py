def _get_rewards(self) -> torch.Tensor:
    ...
    rew_cart_pos = rew_scale_cart_pos * torch.square(torch.abs(cart_pos))
    rew_pole_angle_diff = rew_scale_pole_angle_diff * torch.abs(pole_vel) * torch.abs(torch.cos(pole_pos))
    rewards = {
        "alive": rew_alive,
