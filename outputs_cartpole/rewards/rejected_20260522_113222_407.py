def _get_rewards(self) -> torch.Tensor:
    ...
    rew_pole_angle = rew_scale_pole_pos * torch.clamp(1.0 - torch.abs(pole_pos) / (math.pi / 2), 0.0, 1.0)
    rew_cart_balance = rew_scale_cart_vel * torch.exp(-torch.abs(cart_pos) / self.cfg.max_cart_pos)
    rewards = {
        "alive": rew_alive,
