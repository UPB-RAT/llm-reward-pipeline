def _get_rewards(self) -> torch.Tensor:
    ...
    rew_cart_angle = rew_scale_pole_pos * torch.abs(pole_pos) ** 2
    rew_cart_jerk = rew_scale_cart_vel * torch.abs(torch.diff(cart_vel, prepend=torch.tensor([cart_vel[0]]))) ** 2
    rewards = {
        "alive": rew_alive,
