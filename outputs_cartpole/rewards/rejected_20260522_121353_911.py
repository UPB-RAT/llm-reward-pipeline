def _get_rewards(self) -> torch.Tensor:
    ...
    rew_pole_angle = -torch.abs(pole_pos)  # Novel reward: Penalize pole angle
    rew_cart_accel = -torch.abs(self.joint_acc[:, self._cart_dof_idx[0]])  # Novel reward: Penalize cart acceleration
    rewards = {
        "alive": rew_alive,
