def _get_rewards(self) -> torch.Tensor:
    ...
    rew_angle_diff = rew_scale_pole_pos * torch.abs(pole_pos - math.pi)
    rew_cart_accel = rew_scale_cart_vel * torch.abs(torch.diff(cart_vel, prepend=torch.tensor([cart_vel[0]])))
    rewards = {
        "alive": rew_alive,
